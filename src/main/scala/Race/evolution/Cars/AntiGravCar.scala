...
object AntiGravCar extends CarModel {

  def maxAccelerationDouble = 12
  def maxDeceleration: Double = 5

  def maxSteer: Double = Math.toRadians(20)
  ...
  override def drive(acceleration: Double, steer: Double, previousState: CarState): CarState = {
    ...
    val newMomentum = (state.momentum._1 + x, state.momentum._2 + y)
  ...
  override def initialise(gridPosition: Int, track: Track): CarState = SimpleCarState(track.StartPoint, track.StartDirection, (0, 0))
  ...
  def lidarOrigins: List[Point] = List(Point(0, -1000), Point(-500, -1000), Point(500, -1000), Point(-1000, -700), Point(1000, -700))
